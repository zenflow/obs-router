...
ObsRouter.prototype._update = function(url, emit){
	...
	self.url = url;
	self.route = self.getRouteFromUrl(self.url, self.params = {});
	if (old_route){
...
ObsRouter.prototype.replaceRoute = function(route, params, extend_params){
	this._update(this.getUrlFromRoute(route, params, extend_params), true);
};
ObsRouter.prototype.pushRoute = function(route, params, extend_params){
	var self = this;
	self.pushUrl(self.getUrlFromRoute(route, params, extend_params));
};
...
ObsRouter.prototype.getUrlFromRoute = function(route, params, extend_params){
	var self = this;
	var _route = route || self.route;
	var _params = _.assign({}, extend_params ? self.params : {}, params || {});
	return ObsRouter.getUrlFromRoute(self._routes, _route, _params);
};
ObsRouter.prototype.getRouteFromUrl = function(url, params){
	return ObsRouter.getRouteFromUrl(this._routes, url, params);
};
ObsRouter.getUrlFromRoute = function(routes, route_name, params){
	params = params || {};
...
ObsRouter.getRouteFromUrl = function(routes, url, params){
	var self = this;
	var route_name = null;
